...
DROP DATABASE IF EXISTS `nhasach`;
CREATE DATABASE IF NOT EXISTS `nhasach` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `nhasach`;

-- Dumping structure for procedure bookstore.capnhatgia
DROP PROCEDURE IF EXISTS `capnhatgia`;
DELIMITER //
CREATE PROCEDURE `capnhatgia`(
	IN `ma_sach` VARCHAR(15),
	IN `gia_moi` INT(10)
)
BEGIN
UPDATE sach
...
-- Dumping data for table bookstore.chitiethd: ~5 rows (approximately)
...
-- Dumping structure for table bookstore.hoadon
...
-- Dumping data for table bookstore.hoadon: ~6 rows (approximately)
...
-- Dumping data for table bookstore.loai: ~17 rows (approximately)
...
-- Dumping structure for view bookstore.sachgiacao
DROP VIEW IF EXISTS `sachgiacao`;
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `sachgiacao` (
	`masach` VARCHAR(15) NOT NULL COLLATE 'utf8_unicode_ci',
	`tensach` VARCHAR(250) NOT NULL COLLATE 'utf8_unicode_ci',
	`mota` TEXT NOT NULL COLLATE 'utf8_unicode_ci',
	`gia` INT(10) NOT NULL,
	`hinh` VARCHAR(50) NOT NULL COLLATE 'utf8_unicode_ci',
	`manxb` VARCHAR(5) NOT NULL COLLATE 'utf8_unicode_ci',
	`maloai` VARCHAR(5) NOT NULL COLLATE 'utf8_unicode_ci'
) ENGINE=MyISAM;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `sachgiacao` AS select `sach`.`masach` AS `masach`,`sach`.`tensach` AS `tensach`,`sach`.`mota` AS `mota`,`sach`.`gia` AS `gia`,`sach`.`hinh` AS `hinh`,`sach`.`manxb` AS `manxb`,`sach`.`maloai` AS `maloai` from `sach` order by `sach`.`gia` desc limit 0,2;
